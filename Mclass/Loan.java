...
public class Loan {                                       
    ...
    // ✅ 상수: 모든 문자 대문자 + final static + 언더스코어 구분
    ...
    // ── 정보 전문가 패턴: Loan이 loanDate를 갖고 있으므로 연체 계산도 Loan이 담당 ──
    ...
    /** 현재 대출 중인지 여부 ( null이면 아직 안 돌려준 상태) */
    ...
    public String toCsvString() {
        ...
        return loanId + "," + memberId + "," + bookId + ","
             + loanDate.format(DATE_FORMATTER) + "," + returnDateString;
    }

    /** CSV 한 줄을 Loan 객체로 복원 (static → Loan.fromCsvString(line) 호출) */
    public static Loan fromCsvString(String line) {
        String[] parts = line.split(",");
        LocalDate parsedLoanDate = LocalDate.parse(parts[3], DATE_FORMATTER);
          // LocalDate.parse(문자열, 형식) : 문자열을 LocalDate로 변환
        LocalDate parsedReturnDate = parts[4].equals("null")
                ? null
